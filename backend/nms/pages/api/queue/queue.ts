...
import { Device, DeviceToken } from "../types/types";
...
const worker = new Worker("notificationQueue", async (job) => {
  ...
  if (authors && authors.length > 0) {
    console.log(`Finding devices subscribed to authors: ${authors.map(a => a.name).join(', ')}`);
    ...
    const subscribedDevices = await db('devices')
      .join('device_author_subscriptions', 'devices.id', '=', 'device_author_subscriptions.deviceId')
      .whereIn('device_author_subscriptions.authorId', authorIds)
      .select('devices.expoPushToken')
      .distinct();
    ...
    console.log(`Found ${subscribedDevices.length} additional devices subscribed to authors`);
  }
