...
test("Test Notification Form Elements", async ({ page }) => {
  ...
  // Wait a second for the buttons to load
  ...
  const weeklySummaryCheckboxExists = await page.isVisible(
    'input[value="Weekly Summary"]'
  );
  const dailySummaryCheckboxExists = await page.isVisible(
    'input[value="Daily Summary"]'
  );
...
test("Test Notification Form", async ({ page }) => {
  ...
  // Check if notification is present in the table
...
test("Test Notification Deletion", async ({ page }) => {
  ...
  // Check if notification is no longer present in the table
  const notificationExists = await page.isVisible("text=pending");
