...
describe('device counting endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    if (categories.length === 0) {
      throw new Error('No categories found in database. Make sure migrations have been run.');
    }
  ...
  async function createTestDevice(categoryNames: string[], isPushEnabled: boolean = true) {
    ...
    if (categoryNames.length > 0) {
      ...
      const categories = await db('categories').whereIn('name', categoryNames).select('id');
  ...
  async function setupTestDevices() {
    ...
    await createTestDevice(['Breaking News', 'Sports', 'University News'], true);
    ...
    console.log(`Categorized push-enabled devices: ${categorizedPushEnabledCount}`);
  }
